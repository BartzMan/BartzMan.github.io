...
document.querySelectorAll("[data-quote]").forEach((form) => {
  form.addEventListener("submit", (e) => {
    ...
    const body = `Hi Paradise Green, I'm ${name} in ${city}. ${service ? "Need: "+service+". " : ""}${message} Call me at ${phone}.`;
    const wrap = form.parentElement;
    form.hidden = true;
    const ok = wrap.querySelector(".quote-ok");
    ok.hidden = false;
    ok.querySelector(".sms").href = "sms:+16024211175?&body=" + encodeURIComponent(body);
  });
});
